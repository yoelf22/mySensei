...
export function renderSyllabusHtml({ curriculum, webhookUrl, courseId }) {
  ...
  const subtitle = fm.subtitle || curriculum.angle || "";
  const introHtml = String(fm.introduction || "")
    .split(/\n\s*\n/)
    .map((p) => p.trim())
    .filter(Boolean)
    .map((p) => `<p>${escapeHtml(p)}</p>`)
    .join("");
  const bib = Array.isArray(fm.bibliography) ? fm.bibliography : [];
  const bibHtml = bib
    .map((b) => {
      const t = escapeHtml(b.title || "");
      const author = b.author ? `, ${escapeHtml(b.author)}` : "";
      const note = b.note ? `<div class="muted">${escapeHtml(b.note)}</div>` : "";
      return `<li><b>${t}</b>${author}${note}</li>`;
    })
    .join("");
...
<title>${escapeHtml(L.syllabus)} — ${escapeHtml(title)}</title>
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .bib { list-style:none; padding:0; margin:.5rem 0 0; }
  .bib li { display:block; border-top:1px solid var(--line); padding:.7rem 0; }
  .bib .muted { color:var(--muted); font-family:system-ui,sans-serif; font-size:.9rem; }
...
<main>
  ...
  <p class="angle">${escapeHtml(subtitle)}</p>
  ...
  ${introHtml ? `<h2>${escapeHtml(L.intro)}</h2><section class="intro">${introHtml}</section>` : ""}
  <h2>${escapeHtml(L.contents)}</h2>
  <ol>${items}</ol>
  ${bibHtml ? `<h2>${escapeHtml(L.bibliography)}</h2><ul class="bib">${bibHtml}</ul>` : ""}
...
(function(){
  ...
  f.addEventListener("submit", function(e){
    ...
    fetch(HOOK, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ type:"approve", courseId:${JSON.stringify(courseId || "")} }) })
