...
<script>(function(){
  ...
  function showLockBlock(issues){
    ...
    var txt=issues&&String(issues).trim()?String(issues):"The "+(M.stage==="plan"?"plan":"paper")+" needs more discussion before it's ready to lock. Reply above to keep refining it.";
    var div=document.createElement("div");div.id="lockblock";div.className="lockblock";
    div.innerHTML="<b>mySensei held off on locking.</b> What needs work first:<br>"+esc(txt).replace(/\\n/g,"<br>");
