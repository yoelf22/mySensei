...
export function renderProjectHtml({ courseId, webhookUrl, stage, status, document, thread = [], downloads = null, deck = null, languageCode = "en", ready = false, openQuestions = "" }) {
  ...
  const sendDesc = `Send your reply to mySensei to steer the ${noun}. Does nothing if the box is empty — type something first.`;
  const regenDesc = `Rebuild the ${noun} from scratch using this conversation. Your messages stay, but the current ${noun} document is replaced. Takes about a minute.`;
  const lockDesc = stage === "plan"
    ? "Locks the plan and starts writing the full paper in the background — not interactive; you'll get an email when the draft is ready. You can't steer the plan by chat after this."
    : "Locks the paper and builds the final version + downloads in the background — you'll get an email when they're ready. You can't steer it by chat after this.";
  // mySensei's open questions, shown openly and refreshed every turn. Lock stays
  // disabled until mySensei has none left (ready === true).
  const q = String(openQuestions || "").trim();
  const questionsPanel = (!ready && q)
    ? `<div class="qpanel"><b>mySensei's open questions — answer these in the box below to steer the ${noun}:</b><div class="qbody">${escapeHtml(q).replace(/\n/g, "<br>")}</div></div>`
    : "";
  const lockHint = ready
    ? `<p class="lockhint ok">✓ mySensei has no open questions left — you can lock the ${noun} now.</p>`
    : `<p class="lockhint">mySensei still has open questions${q ? " (above)" : ""}. You can lock the ${noun} once they're resolved.</p>`;
  const controlsInner = locked ? `${dl}${deckBtn}${building}${deckPanel}` : `
    ${questionsPanel}
    <textarea id="msg" placeholder="Answer mySensei to steer the ${noun}..."></textarea>
    <div class="actions">
      <div class="action"><button id="send" class="b-send">Send</button><span class="desc">${sendDesc}</span></div>
      <div class="action"><button data-act="regenerate" class="b-regen">Regenerate ${noun}</button><span class="desc">${regenDesc}</span></div>
...
.qpanel{background:#fff;border:1px solid var(--line);border-inline-start:4px solid #b4541f;border-radius:.5rem;padding:.85rem 1rem;margin:1rem 0;font:14px/1.55 system-ui,sans-serif}
.qpanel>b{display:block;margin-bottom:.4rem;color:#b4541f}
.qbody{color:var(--ink)}
...
.lockblock{background:#fff;border:1px solid var(--line);border-inline-start:4px solid #c0392b;border-radius:.5rem;padding:.8rem .95rem;margin-bottom:.8rem;font:14px/1.5 system-ui,sans-serif}
.lockblock b{color:#c0392b}
...
<script>(function(){
  ...
  // input is accepted until it resolves. When unfrozen, Lock stays disabled
  // unless mySensei has marked the work ready (data-ready="1").
  function setBusy(on){
  ...
  function onAct(){
    ...
    if(act==="lock"&&!confirm("Lock the "+noun+" and start writing the paper? mySensei has no open questions left. This ends the conversation - you can't steer the "+noun+" by chat after this, and you'll get an email when the draft is ready."))return;
    if(act==="regenerate"&&!confirm("Rebuild the "+noun+" from this conversation? Your messages stay, but the current "+noun+" document is replaced."))return;
    setBusy(true);
    if(act==="lock"){
      var wl=toast("Locking the "+noun+" and starting the paper...","",{sticky:true,spin:true});
      post({type:"lock",courseId:M.courseId,stage:M.stage}).then(function(res){
        hide(wl);
        if(res&&res.locked===false){
          // Backstop: mySensei changed its mind (e.g. a stale tab). Pull the
          // latest so its open questions reappear; don't promise an email.
          toast("mySensei has open questions again - see them above. Lock will re-enable once they're resolved.","error",{ms:9000});
          setBusy(false);watch();
          return;
        }
        ...
        toast("Locked. Writing your "+(M.stage==="plan"?"paper":"final paper")+" in the background - we'll email you when it's ready.","ok",{ms:11000});
        watch();
      }).catch(function(){hide(wl);toast("Could not reach mySensei - please try again.","error");setBusy(false);});
      return;
    }
