...
<script>(function(){
  ...
  // Poll this same page until its content fingerprint changes (the background
  // job has re-rendered), then reload so the new reply/result shows by itself.
  var watching=false;
  function watch(){if(watching)return;watching=true;var tries=0,max=75;
    (function loop(){setTimeout(function(){tries++;
      fetch(location.href,{cache:"no-store"}).then(function(r){return r.text();}).then(function(html){
        var m=html.match(/<script id="meta"[^>]*>([\\s\\S]*?)<\\/script>/);var changed=false;
        if(m){try{changed=String(JSON.parse(m[1]).sig)!==String(M.sig);}catch(e){}}
        if(changed){toast("Ready - updating now","ok");setTimeout(function(){location.reload();},700);return;}
        if(tries<max)loop();else{watching=false;toast("Still working - this is taking a while. The page will update on its own when it's ready.","ok",{ms:9000});}
      }).catch(function(){if(tries<max)loop();else watching=false;});
    },4000);})();}
  var send=document.getElementById("send");
  if(send)send.addEventListener("click",function(){
    var b=document.getElementById("msg");var v=b.value.trim();
    if(!v){toast("Type a reply first, then press Send.","error");b.focus();return;}
    send.disabled=true;var w=toast("Sending your reply...","",{sticky:true,spin:true});
    post({type:"dialogue",courseId:M.courseId,stage:M.stage,text:v}).then(function(){
      hide(w);toast("Sent. mySensei is thinking - the page will update when the reply is ready.","",{spin:true,ms:6000});watch();
    }).catch(function(){hide(w);toast("Could not send - please try again.","error");send.disabled=false;});
  });
  document.querySelectorAll("button[data-act]").forEach(function(b){
    b.addEventListener("click",function(){
      ...
      post({type:act,courseId:M.courseId,stage:M.stage}).then(function(){
        hide(w);toast("Started. mySensei is working - the page will update when it's ready.","",{spin:true,ms:6000});watch();
