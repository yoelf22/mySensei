...
function renderDrills(drills = [], labels) {
  if (!drills || !drills.length) return "";
  const items = drills
    .map((d, i) => {
      const prompt = `<div class="drill-prompt">${(String(d.prompt || "").split(/\n+/).filter(Boolean).map((p) => `<p>${escapeHtml(p)}</p>`).join("")) || ""}</div>`;
      const solBody = String(d.solution || "").split(/\n+/).filter(Boolean).map((p) => `<p>${escapeHtml(p)}</p>`).join("");
      const sol = solBody
        ? `<details class="drill-sol"><summary>${escapeHtml(labels.showSolution)}</summary>${solBody}</details>`
        : "";
      return `<li class="drill"><span class="drill-n">${i + 1}</span><div class="drill-body">${prompt}${sol}</div></li>`;
    })
    .join("");
  return `<section class="drills"><h2>${escapeHtml(labels.drillsTitle)}</h2><ol class="drill-list">${items}</ol></section>`;
}
 ...
 */
export function renderLessonHtml({ curriculum, lesson, webhookUrl, courseId }) {
  ...
  const meta = {
    ...
    concepts: (lesson.quiz || []).map((q) => q.concept || ""),
    explanations: (lesson.quiz || []).map((q) => q.explanation || ""),
  ...
  const body = `
    ...
    ${renderDrills(lesson.drills, labels)}
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .drills h2 { color:var(--accent); }
  .drill-list { list-style:none; padding:0; margin:1rem 0; counter-reset:none; }
  .drill { display:flex; gap:.8rem; background:#fff; border:1px solid var(--line); border-radius:.5rem; padding:1rem 1.1rem; margin:.8rem 0; }
  .drill-n { flex:0 0 1.6rem; height:1.6rem; border-radius:50%; background:var(--accent); color:#fff; font:bold .9rem/1.6rem system-ui,sans-serif; text-align:center; }
  .drill-body { flex:1; }
  .drill-prompt p:first-child { margin-top:0; }
  .drill-sol { margin-top:.6rem; font-family:system-ui,sans-serif; font-size:.95rem; }
  .drill-sol summary { cursor:pointer; color:var(--accent); font-weight:bold; }
  .drill-sol[open] summary { margin-bottom:.4rem; }
  ...
  .opt.correct span { color:#1a7f37; font-weight:bold; }
  .opt.wrong span { color:var(--accent); text-decoration:line-through; }
  .fb { font-family:system-ui,sans-serif; font-size:.92rem; margin:.5rem 0 0; }
  .fb.ok { color:#1a7f37; } .fb.no { color:var(--accent); }
  button { font:inherit; background:var(--accent); color:#fff; border:0; border-radius:.4rem; padding:.7rem 1.4rem; cursor:pointer; }
  #result { font-family:system-ui,sans-serif; font-weight:bold; font-size:1.05rem; line-height:1.45; margin-top:1.25rem; padding:.9rem 1.1rem; border-radius:.5rem; border-inline-start:5px solid currentColor; animation:result-in .25s ease-out; }
  #result:empty { display:none; padding:0; margin:0; border:0; }
  #result.ok { background:#e7f4ea; }
  #result.no { background:#fbeadf; }
  @keyframes result-in { from { opacity:0; transform:translateY(.3rem); } to { opacity:1; transform:none; } }
...
(function () {
  ...
  form.addEventListener("submit", function (e) {
    ...
    for (var i = 0; i < meta.total; i++) {
      if (!form.querySelector('input[name="q' + i + '"]:checked')) { out.textContent = L.answerAll; out.className = "no"; return; }
    }
    var fsList = form.querySelectorAll("fieldset.q");
    var score = 0, missed = [];
    for (var i = 0; i < meta.total; i++) {
      var fs = fsList[i];
      var chosen = parseInt(fs.querySelector('input[name="q' + i + '"]:checked').value, 10);
      var ok = chosen === meta.correct[i];
      if (ok) score++; else if (meta.concepts[i]) missed.push(meta.concepts[i]);
      var opts = fs.querySelectorAll("label.opt");
      for (var k = 0; k < opts.length; k++) { opts[k].querySelector("input").disabled = true; }
      if (opts[chosen]) opts[chosen].className += ok ? " correct" : " wrong";
      if (!ok && opts[meta.correct[i]]) opts[meta.correct[i]].className += " correct";
      var correctText = opts[meta.correct[i]] ? opts[meta.correct[i]].querySelector("span").textContent : "";
      var line = ok ? "✓" : ("✗ " + L.correctAnswer + " " + correctText);
      if (meta.explanations[i]) line += " — " + meta.explanations[i];
    ...
    fetch(meta.webhook, {
      ...
      body: JSON.stringify({ type: "quiz", courseId: meta.courseId, module: meta.module, attempt: meta.attempt, score: score, total: meta.total, passed: passed, missed: missed }),
    }).then(function () {
