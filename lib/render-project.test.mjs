...
test("lock is disabled until mySensei is ready, enabled when ready", () => {
  const notReady = renderProjectHtml(base);
  assert.match(notReady, /id="lockbtn"[^>]*disabled/);
  const ready = renderProjectHtml({ ...base, ready: true });
  assert.doesNotMatch(ready, /id="lockbtn"[^>]*disabled/);
  assert.match(ready, /no open questions left/);
});
test("open questions are shown openly when present and not ready", () => {
  const html = renderProjectHtml({ ...base, openQuestions: "- Which thesis?\n- Why this mechanism?" });
  assert.match(html, /<div class="qpanel">/);
  assert.match(html, /Which thesis\?/);
  assert.match(html, /Why this mechanism\?/);
});
test("open questions panel hidden once ready", () => {
  const html = renderProjectHtml({ ...base, ready: true, openQuestions: "- leftover" });
  assert.doesNotMatch(html, /<div class="qpanel">/);
});
