...
export function renderAssessmentHtml({ questions = [], webhookUrl, courseId, languageCode = "en", subject = "" }) {
...
<style>
  ...
  *{box-sizing:border-box;} body{margin:0;background:var(--bg);color:var(--ink);font:17px/1.6 Georgia,serif;}
  ...
  #done{display:none;margin-top:1.3rem;font-family:system-ui,sans-serif;font-weight:bold;font-size:1.05rem;line-height:1.45;color:#1a7f37;background:#e7f4ea;border-inline-start:5px solid currentColor;border-radius:.5rem;padding:.9rem 1.1rem;animation:banner-in .25s ease-out;}
  #err{display:none;margin-top:1rem;font-family:system-ui,sans-serif;font-weight:bold;color:var(--accent);background:#fbeadf;border-inline-start:5px solid currentColor;border-radius:.5rem;padding:.9rem 1.1rem;animation:banner-in .25s ease-out;}
  @keyframes banner-in{from{opacity:0;transform:translateY(.3rem);}to{opacity:1;transform:none;}}
</style>
</head>
<body>
<main>
  <h1>Quick placement check</h1>
  <p class="sub">${escapeHtml(subject)} — a few hands-on tasks, easy to hard. Work each one out; this just sets how deep your lessons start. Then I'll build your course and email it.</p>
  <form id="f">${blocks}
...
(function(){
  ...
  f.addEventListener("submit", function(e){
    ...
    fetch(HOOK,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"assessment",courseId:${JSON.stringify(courseId || "")},results:results})})
