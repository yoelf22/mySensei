...
export function renderProjectHtml({ courseId, webhookUrl, stage, status, document, thread = [], downloads = null, deck = null, languageCode = "en" }) {
  ...
  const readyLabel = `I'm done refining — I'm ready to finish the ${noun}.`;
  const controlsInner = locked ? `${dl}${deckBtn}${building}${deckPanel}` : `
    ...
    <div class="actions">
      ...
      <div class="action lockgate">
        <label class="ready"><input type="checkbox" id="lockready"> ${readyLabel}</label>
        <button data-act="lock" id="lockbtn" class="b-lock" disabled>${lockLabel}</button>
        <span class="desc">${lockDesc} <b>Tick the box above to enable this.</b></span>
      </div>
    </div>`;
...
.lockgate .ready{display:block;font:14px/1.45 system-ui,sans-serif;color:var(--ink);margin-bottom:.5rem;cursor:pointer}
.lockgate .ready input{width:auto;margin-inline-end:.45rem;vertical-align:-.1em}
...
<script>(function(){
  ...
  function setBusy(on){
    ...
    if(!on){var cb=document.getElementById("lockready"),lk=document.getElementById("lockbtn");if(cb&&lk)lk.disabled=!cb.checked;}
  }
  function onSend(){
  ...
  function wire(){
    ...
    var cb=document.getElementById("lockready"),lk=document.getElementById("lockbtn");
    if(cb&&lk){lk.disabled=!cb.checked;cb.addEventListener("change",function(){lk.disabled=!cb.checked;});}
  }
  wire();
})();</script>
</body></html>`;
