...
// A stalled request must fail fast, not hang on the SDK's 10-minute default
// (which, with 2 retries, can wedge a job for ~30 min of silence).
const TIMEOUT_MS = Number(process.env.ANTHROPIC_TIMEOUT_MS) || 240000;

export function client() {
  return new Anthropic({ maxRetries: 2, timeout: TIMEOUT_MS }); // reads ANTHROPIC_API_KEY
}
...
export async function structured(c, prompt, schema, maxTokens = 4000, model = MODEL) {
  const r = await c.messages.create({
    model,
...
export function heavyClient() {
  return new Anthropic({ maxRetries: 2, timeout: TIMEOUT_MS }); // reads ANTHROPIC_API_KEY; model chosen per-call
}
...
export function extractSources(content) {
  ...
  for (const b of content || []) {
    ...
    if (b.type === "text" && Array.isArray(b.citations)) {
      for (const ci of b.citations) if (ci) add(ci.url, ci.title);
    }
  }
  return out;
}
