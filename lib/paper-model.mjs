...
export const PAPER_OUTLINE_SCHEMA = {
  ...
  properties: {
    ...
    headings: { type: "array", items: { type: "string" }, minItems: 3, maxItems: 6 },
    conclusionHint: { type: "string" },
  },
  required: ["title", "headings"],
};

export function outlinePrompt({ planText, settings = {}, thread = [] }) {
  const convo = thread.length
    ? `\n\nThe author discussed the draft with you — REVISE to reflect the conversation:\n` +
...
export function sectionPrompt({ subject, settings = {}, planText, heading, priorText = "", thread = [] }) {
  const convo = thread.length
    ? `\n\nThe author discussed the draft with you — REVISE to reflect the conversation:\n` +
      thread.map((m) => `${m.role === "user" ? "Author" : "You"}: ${m.content}`).join("\n")
    : "";
...
export function conclusionPrompt({ subject, settings = {}, planText, bodyText = "", thread = [] }) {
  const convo = thread.length
    ? `\n\nThe author discussed the draft with you — REVISE to reflect the conversation:\n` +
      thread.map((m) => `${m.role === "user" ? "Author" : "You"}: ${m.content}`).join("\n")
    : "";
