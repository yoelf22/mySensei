...
export function renderProjectHtml({ courseId, webhookUrl, stage, status, document, thread = [], downloads = null, deck = null, languageCode = "en", ready = false }) {
  ...
  const lockHint = ready
    ? `<p class="lockhint ok">✓ mySensei thinks this ${noun} is solid. Click <b>${lockLabel}</b> to finish and start the paper.</p>`
    : `<p class="lockhint">Ready to finish? Click <b>${lockLabel}</b>. mySensei reviews it first — if anything's missing it tells you what to fix before writing starts.</p>`;
  const controlsInner = locked ? `${dl}${deckBtn}${building}${deckPanel}` : `
    ...
    <div class="actions">
      ...
      <div class="action lockgate">
        ${lockHint}
        <button data-act="lock" id="lockbtn" class="b-lock">${lockLabel}</button>
...
<script>(function(){
  ...
  function showLockBlock(issues){
    var host=document.querySelector(".lockgate");if(!host)return;
    var old=document.getElementById("lockblock");if(old&&old.parentNode)old.parentNode.removeChild(old);
    var noun=M.stage==="plan"?"plan":"paper";
    var div=document.createElement("div");div.id="lockblock";div.className="lockblock";
    if(issues&&String(issues).trim()){
      div.innerHTML="<b>mySensei held off on locking.</b> Answer these in the box above, then click Lock the "+noun+" again:<br>"+esc(String(issues)).replace(/\\n/g,"<br>");
    }else{
      div.innerHTML="<b>mySensei held off on locking.</b> It hasn't reviewed this version yet — send a reply above and it will tell you exactly what to answer before you can lock the "+noun+".";
    }
  ...
  function onAct(){
    ...
    var noun=M.stage==="plan"?"plan":"paper";
    if(act==="lock"&&!confirm("Ask mySensei to lock the "+noun+" and start writing? It reviews first - if the "+noun+" is ready, this ends the conversation and the paper is written in the background (you'll get an email)."))return;
    ...
    setBusy(true);
    if(act==="lock"){
      var wl=toast("Asking mySensei to review and lock the "+noun+"...","",{sticky:true,spin:true});
      post({type:"lock",courseId:M.courseId,stage:M.stage}).then(function(res){
        hide(wl);
        if(res&&res.locked===false){
          showLockBlock(res.issues);
          toast("mySensei held off on locking - see what needs work below.","error",{ms:9000});
          setBusy(false);
          return;
        }
